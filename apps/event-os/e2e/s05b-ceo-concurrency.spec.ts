...
test("S05B two-tab concurrency keeps the later stale write from silently winning", async ({ browser }) => {
  ...
  await formA.getByLabel("Insurer party id").fill("00000000-0000-4000-8000-000000000202");
  await formA.getByLabel("Insurer label").fill("Tab one insurer");
  await formB.getByLabel("Policy type").selectOption("EVENT_CANCELLATION");
  await formB.getByLabel("Insurer party id").fill("00000000-0000-4000-8000-000000000202");
  await formB.getByLabel("Insurer label").fill("Tab two insurer");
  ...
  await expect(first.page.getByTestId("protection-command")).toBeVisible({ timeout: 20_000 });
  await expect(first.page.locator('select[name="policyId"] option', { hasText: "Tab one insurer" })).toHaveCount(1);
  await expect(first.page.locator('select[name="policyId"] option', { hasText: "Tab two insurer" })).toHaveCount(1);
