...
import { ALPHA_PROTECTION, assembleWorkingDraft, evaluateAlphaOne, expectActionOutcome } from "./s060-helpers";
...
test("S061 governing authority, publication and client access", async ({ page, browser }) => {
  ...
  await expect(page.locator("li", { hasText: "Later discovery draft must remain history." })).toContainText(/history/i);

  const reviewer3 = await openStaffContext(browser, "reviewer");
  await reviewer3.page.goto("/app/protection");
