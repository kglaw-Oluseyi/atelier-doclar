...
export default async function ForecastPage({
  ...
  const state = operationalStateFromQuery({
    error: typeof query.error === "string" ? query.error : flash?.message,
    state: typeof query.state === "string" ? query.state : flash?.code,
  ...
  return (
    <AppShell person={person} organisationName={scoped.organisation.displayName} eventName={scoped.event.name} current="/app/events">
      ...
      <ForecastWorkspace workspace={workspace} mutationLocked={state?.reloadRequired === true} />
    </AppShell>
  );
}
