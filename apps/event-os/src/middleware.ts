...
export function middleware(request: NextRequest): NextResponse {
  ...
  if (pathname.startsWith("/api/_diag/") || pathname.startsWith("/api/s073-diag/")) {
    return withPrivateCache(new NextResponse(null, { status: 404 }));
  }
...
export const config = {
  matcher: ["/", "/app/:path*", "/access-:path*", "/api/:path*", "/sign-in", "/rsvp", "/rsvp/:path*", "/vendor", "/vendor/:path*", "/offers", "/offers/:path*", "/atelier", "/atelier/:path*", "/client-dossier", "/client-dossier/:path*"],
};
