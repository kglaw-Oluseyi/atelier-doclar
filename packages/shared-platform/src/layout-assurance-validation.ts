...
function finding(
  ...
  return {
    ...
    objectIds: input.objectIds.slice(0, 200),
    evidence: input.evidence.length > 800 ? `${input.evidence.slice(0, 777)}… truncated` : input.evidence,
    explanation: input.explanation.slice(0, 800),
    recommendedAction: input.recommendedAction.slice(0, 400),
