...
export function publishAtelierOnSnap(
  ...
  publishDraft(snap.eventNarrativeEditions);
...
export function publishNarrativeEditionOnSnap(
  ...
  if (current && current.publicationState === "PUBLISHED") {
    current.publicationState = "SUPERSEDED";
    current.version += 1;
...
export function revokeAtelierAccessOnSnap(
  ...
  grant.version += 1;
  grant.updatedAt = now;
  for (const session of snap.atelierSessions.filter((item) => item.grantId === grant.id && item.status === "ACTIVE")) {
    session.status = "REVOKED";
    session.revokedAt = now;
    session.version += 1;
    session.updatedAt = now;
  }
  for (const challenge of snap.magicLinkChallenges.filter((item) => item.grantId === grant.id && item.status === "ISSUED")) {
